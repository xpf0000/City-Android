...
/**
 * Created by X on 2016/12/9.
 */

public class MineHeaderTouchListener implements RecyclerView.OnItemTouchListener {
    ...
    public interface OnHeaderClickListener {
        void onHeaderClick(int flag);
    }

    public MineHeaderTouchListener(final RecyclerView recyclerView,
                                              final StickyRecyclerHeadersDecoration decor) {
        mTapDetector = new GestureDetector(recyclerView.getContext(), new MineSingleTapDetector());
    ...
    private class MineSingleTapDetector extends GestureDetector.SimpleOnGestureListener {
        @Override
        public boolean onSingleTapUp(MotionEvent e) {
            ...
            if (position != -1) {
                ...
                MyMineAdapter adapter = (MyMineAdapter) getAdapter();
                if((int)e.getX() <= LocationApplication.SW * 0.5)
                {
                    mOnHeaderClickListener.onHeaderClick(0);
                }
                else
                {
                    mOnHeaderClickListener.onHeaderClick(1);
                }
